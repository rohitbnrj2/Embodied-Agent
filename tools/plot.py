...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Plot results from multiple log folders"
    )
    parser.add_argument(
        "--parent_folder", "-p", type=str, help="Parent folder", default=None
    )
    parser.add_argument(
        "--log_folders", "-l", nargs="+", help="List of log folders to plot", default=[]
    )
    parser.add_argument("--name", help="Path to save the plot")
    parser.add_argument(
        "--fill_between", action="store_true", help="Fill between the curves"
    )
    parser.add_argument(
        "--shorten_label",
        action="store_true",
        help="Shorten Label to the first 20 characters",
    )
    args = parser.parse_args()

    # parent and log folders are mutually exclusive
    assert not (
        args.parent_folder and args.log_folders
    ), "Only one of parent_folder and log_folders can be set"

    if args.parent_folder:
        # get all directories in parent_folder
        args.log_folders = [
            f"{args.parent_folder}/{d}"
            for d in os.listdir(args.parent_folder)
            if os.path.isdir(f"{args.parent_folder}/{d}")
        ]

...
def rs_prop(u1, z, FX, FY, lmbda):
    k = 2 * np.pi / lmbda
    H_valid = (np.sqrt(FX**2 + FY**2) < 1.0 / lmbda).astype(np.float32)
    H = H_valid * np.nan_to_num(
        np.exp(1j * k * z * np.sqrt(1.0 - (lmbda * FX) ** 2 - (lmbda * FY) ** 2))
    )
...
class MjCambrianNonDifferentiableOptics(torch.nn.Module):
    ...
    def reset(self, config: "MjCambrianEyeConfig"):
        self.config = config
        self.A, self.X1, self.Y1, self.FX, self.FY, self.focal = self.define_simple_psf(
            config
        )

    def forward(
        self, image: np.ndarray, depth: np.ndarray, return_psf: bool = False
    ) -> np.ndarray:
        ...
        img.append(
            torch.nn.functional.conv2d(
                image[:, 0, :, :], psf[:, 0, None, :, :], padding="same"
            )
        )
        img.append(
            torch.nn.functional.conv2d(
                image[:, 1, :, :], psf[:, 1, None, :, :], padding="same"
            )
        )
        img.append(
            torch.nn.functional.conv2d(
                image[:, 2, :, :], psf[:, 2, None, :, :], padding="same"
            )
        )
        ...
        if self._debug:
            ...
            psf = psf.squeeze(0).permute(1, 2, 0).cpu().numpy()
            psf[:, :, 0] = (psf[:, :, 0] - np.min(psf[:, :, 0])) / (
                np.max(psf[:, :, 0]) - np.min(psf[:, :, 0])
            )
            psf[:, :, 1] = (psf[:, :, 1] - np.min(psf[:, :, 1])) / (
                np.max(psf[:, :, 1]) - np.min(psf[:, :, 1])
            )
            psf[:, :, 2] = (psf[:, :, 2] - np.min(psf[:, :, 2])) / (
                np.max(psf[:, :, 2]) - np.min(psf[:, :, 2])
            )
            fig, axs = plt.subplots(2, 2)
            # set title of the whole plot
            axs[0, 0].imshow(image.squeeze(0).permute(1, 2, 0).cpu().numpy())
            axs[0, 0].set_title(f"GT Image")  # axs[0].axis('off')``
            axs[0, 1].imshow(psf)
            axs[0, 1].set_title("PSF")  # axs[1].axis('off')
            axs[1, 0].imshow(img)
            axs[1, 0].set_title(
                f"Simulated Image with Aperture: {self.config.aperture_open:2f}"
            )  # axs[0].axis('off')
            img = _crop(img, (self.config.resolution[0], self.config.resolution[1]))
            img = Image.fromarray((img * 255).astype(np.uint8))
            img = np.array(img).astype(np.float32) / 255.0
            axs[1, 1].imshow(img)
            axs[1, 1].set_title(f"Eye Image")  # axs[0].axis('off')
    ...
    def define_simple_psf(self, config: "MjCambrianEyeConfig") -> torch.Tensor:
        ...
        fy = np.linspace(-1.0 / (2.0 * dx), 1.0 / (2.0 * dx), My)
...
if __name__ == "__main__":
    ...
    for i in range(len(res)):
        ...
        apertures = [0.0, 0.01, 0.1, 0.25, 0.5, 1.0]
        for aperture in apertures:
            ...
            axs[0].imshow(img)
            axs[0].set_title(
                f"Image After PSF Applied (Aperture: {config.aperture_open:2f})"
            )
            # axs[0].axis('off')
            axs[1].imshow(psf)
            axs[1].set_title("PSF")
        ...
        imageio.mimsave(
            f"./{_save_path}/psf-ims-{SSNAME}.gif", images, loop=4, duration=1
        )
